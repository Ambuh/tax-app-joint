...
const loadIncomeMenu=_=>{
    ...
    cont.generalTags("<button class='app-left app-border'>Download W2</button>");

    cont.generalTags("<button class='app-left app-border'>Download 1040</button>");
    ...
    for(let i=0;i<editCell.length;i++){
        editCell[i].addEventListener('click',function () {
            ...
            database.selectQuery(['*'],'py_income').then(rows=>{
                showPopUp(editIncomeUi(rows, editCell[i].getAttribute('data-id')),()=>{
                    const cancel=document.querySelector("#canChanges");

                    const update=document.querySelector("#mkChanges");
    ...
    document.getElementById('config_liabs').addEventListener('click',()=>{
        dataContainer.innerHTML=loadIncomeConfigLiabilitiesLayout();

        document.getElementById('addCategoryLiabs').addEventListener('click',function () {
            dataContainer.innerHTML=loadIncomeConfigLiabilitiesNewCategoryLiabs();
    ...
    document.getElementById('config_assets').addEventListener('click',()=>{
        dataContainer.innerHTML=loadIncomeConfigAssetsLayout();
        document.getElementById('assetsCategory').addEventListener('click',function () {
          dataContainer.innerHTML=loadIncomeConfigNewAssetsLayout();
...
},loadIncomeConfigLiabilitiesLayout=_=>{
    const cont=new objectString();
    ...
    database.selectQuery(['*'],'py_income_categories').then(liabs=>{
       ...
       liabs.forEach( (liability,key)=>{
           if(liability.c_type==0) {
               ...
               list.addDataRow('1', loadAllSubCategories(liability.description,liability.id,liability.c_type));
               i++;
           }
       })
...
},loadIncomeConfigAssetsLayout=_=>{
    const cont= new objectString();
    ...
    database.selectQuery(['*'],'py_income_categories').then(categories=>{
        ...
        categories.forEach(category=>{
            ...
            if(category.c_type=='1'){
                ...
                list.addDataRow(category.id, loadAllSubCategories(category.description,category.id,category.c_type));
                i++;
            }
        });
    ...
    return cont.toString();
},loadIncomeConfigLiabilitiesTableFunctions=_=>{
...
},loadIncomeConfigLiabilitiesNewCategoryLiabs=_=>{
    const cont= new objectString();
    ...
    return cont.toString();
},loadIncomeConfigNewAssetsLayout=_=>{
    const cont= new objectString();
    ...
    cont.generalTags(loadIncomeLiabilitiesDefaultViewLayout());
  ...
  return cont.toString();
},loadIncomeConfigNewLiabilitiesLayout=_=>{
    const cont= new objectString();
    ...
    return cont.toString();
},loadIncomeLiabilitiesNewMicroFunctions=_=>{
    ...
    document.getElementById('v_liabs').addEventListener('click',function () {
        liabs_bodiesContainer.innerHTML=loadIncomeLiabilitiesDefaultViewLayout();
    });
    document.getElementById('ad_liabs').addEventListener('click',()=>{

        liabs_bodiesContainer.innerHTML=loadIncomeConfigNewLiabilitiesLayout();
...
},loadIncomeAssetsDefaultLayout=_=>{
    const cont= new objectString();
    ...
    cont.generalTags(loadIncomeAssetsDefaultViewLayout());

    cont.generalTags("</div>")

    return cont.toString();
},loadIncomeAssetsDefaultViewLayout=_=>{
    const cont = new objectString();
    ...
    return cont.toString();
},loadIncomeConfigNewAssetsDataLayout=_=>{
    ...
    //cont.generalTags("<div id='expensAdd' style='margin-right: 14% !important;' class='app-right app-margin-right app-button-shape'><i class='fas fa-plus-circle'></i>Record</div>");
    ...
    document.getElementById('crt-charge').addEventListener('click',function (e) {
        ...
        hidden_container.innerHTML=loadChargesLayout();
